...
namespace Yagasoft.Libraries.EnhancedOrgService.ExecutionPlan.SerialiseWorkarounds
{
	...
	internal class DateTimeCrmContractSurrogateCustom : SerialiserHelpers.DateTimeCrmContractSurrogate
	{
		protected override object CustomIterator(object obj)
		{
			if (obj is MockDictionary mockDictionary)
			{
				foreach (var item in mockDictionary.ToArray())
				{
					mockDictionary[item.Key] = ConvertDate(item.Value as string) ?? item.Value;
				}
			}
